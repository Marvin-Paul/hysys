...
const phases: Phase[] = [
  { number: 1, name: 'Planning & Information Architecture', status: 'complete',    progress: 100, description: 'PRD, sitemap, 11 modules, 12 sectors, route redirects' },
  { number: 2, name: 'Design System & Components',          status: 'complete',    progress: 100, description: 'Marmidon WDD tokens, Inter type scale, form & button utilities' },
  { number: 3, name: 'ERP Catalog & Routing',               status: 'complete',    progress: 100, description: 'Module/sector pages, legacy redirects, demo CTAs' },
  { number: 4, name: 'Content & SEO Migration',             status: 'complete',    progress: 100, description: 'ERP copy, JSON-LD, breadcrumbs, sitemap, meta titles' },
  { number: 5, name: 'CMS & Supabase Integration',          status: 'complete',    progress: 100, description: 'Content Manager wired to live pages (Solutions, stats, demo, products)' },
  { number: 6, name: 'i18n & Launch Polish',                status: 'complete', progress: 100, description: 'Lead forms, analytics events, consent, search scope, mega-menu IA' },
  { number: 7, name: 'Production Launch',                   status: 'pending',     progress: 0,   description: 'GTM live, DNS, SSL, stakeholder sign-off' },
];

const completedFeatures: Feature[] = [
  { name: 'Public Marketing Website',     icon: <Globe className="w-4 h-4" /> },
  { name: '11 ERP Module Pages',          icon: <Layers className="w-4 h-4" /> },
  { name: '12 Industry Solution Pages',   icon: <Building2 className="w-4 h-4" /> },
  { name: 'Request a Demo Flow',          icon: <TrendingUp className="w-4 h-4" /> },
  { name: 'CMS Content Manager (page-aligned)', icon: <FileCode className="w-4 h-4" /> },
  { name: 'SEO & Structured Data',        icon: <Shield className="w-4 h-4" /> },
  { name: 'WDD Typography & Spacing (Doc 2 §3)', icon: <Layers className="w-4 h-4" /> },
  { name: 'Per-page SEO Title Matrix (Doc 5 §8)', icon: <FileCode className="w-4 h-4" /> },
  { name: 'Customer Stories & Blog',      icon: <Users className="w-4 h-4" /> },
  { name: 'Multi-language Support',       icon: <Globe className="w-4 h-4" /> },
  { name: 'ERP Chatbot Assistant',        icon: <Zap className="w-4 h-4" /> },
  { name: 'Lead Form Persistence (demo/partner)', icon: <TrendingUp className="w-4 h-4" /> },
  { name: 'Customer Story Filters & Mega-menu IA', icon: <Navigation className="w-4 h-4" /> },
  { name: 'Analytics Event Taxonomy (Doc 4)',     icon: <BarChart3 className="w-4 h-4" /> },
  { name: 'Module FAQ & Sector Proof Blocks',    icon: <Layers className="w-4 h-4" /> },
];

const inProgressFeatures: Feature[] = [
  { name: 'GTM container ID in production .env', icon: <BarChart3 className="w-4 h-4" /> },
  { name: 'Stakeholder content review',   icon: <Users className="w-4 h-4" /> },
  { name: 'Production deployment',        icon: <Server className="w-4 h-4" /> },
  { name: 'Mobile QA spot-checks',        icon: <Globe className="w-4 h-4" /> },
];
...
const nextSteps = [
  { step: 1, action: 'Set VITE_GTM_ID in .env (see .env.example) and verify consent-gated analytics events.' },
  { step: 2, action: 'Stakeholder review of homepage, nav mega-menu, and module detail pages.' },
  { step: 3, action: 'Mobile QA — forms, mega-menu drawer, and module grids on small screens.' },
  { step: 4, action: 'Production deployment — DNS, SSL, and robots/sitemap verification.' },
  { step: 5, action: 'Post-launch: monitor demo form submissions and search console indexing.' },
];
  ...
  return (
    <div className="min-h-screen bg-white print:bg-white" style={{ fontFamily: 'var(--font-family)' }}>
      ...
      <PageHero
        ...
        eyebrow="Marmidon Global Solutions Limited"
        title="Project Status Report"
        subtitle="Current development progress"
        description="Track the Marmidon Enterprise ERP website migration — modules, sectors, content, and launch readiness."
      ...
      <main className="max-w-5xl mx-auto px-8 py-10 space-y-12">
          ...
          <div className="px-8 py-10">
            <div className="flex items-start justify-between flex-wrap gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[var(--color-accent)] text-xs font-semibold uppercase tracking-widest mb-0.5">Marmidon Global Solutions</p>
                  <h2 className="text-2xl font-extrabold leading-tight">Project Status Report</h2>
                  <p className="text-white/70 text-sm mt-0.5">Enterprise ERP Marketing Website</p>
                </div>
              </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {[
              { label: 'Project Name',    value: 'Marmidon ERP Marketing Website',     icon: <FileCode className="w-4 h-4" /> },
              { label: 'Client',          value: 'Marmidon Global Solutions',          icon: <Building2 className="w-4 h-4" /> },
              ...
              { label: 'Current Phase',   value: 'Phase 4–6 (Content + CMS + i18n)', icon: <GitBranch className="w-4 h-4" /> },
              { label: 'Target Launch',   value: 'Q3 2026',                         icon: <Clock className="w-4 h-4" /> },
      ...
      <footer className="border-t border-gray-200 bg-gray-50 mt-8 print:mt-4">
        <div className="max-w-5xl mx-auto px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-400">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-[var(--color-primary)]">Marmidon Global Solutions</span>
            <span>·</span>
            <span>ERP Marketing Website — Internal Project Report</span>
          </div>
          <div className="flex items-center gap-2">
